...
import { AddMatchingFundsModalProps } from '@/utils/modals'
import { useRoundStore } from '@/stores/useRoundStore'
import type { TransactionReceipt } from '@ethersproject/abstract-provider'
import { useFactoryStore } from '@/stores/useFactoryStore'

const props = withDefaults(defineProps<AddMatchingFundsModalProps>(), {})
...
async function onClickTransfer() {
	...
	try {
		...
		if (props.target === 'factory') {
			const factoryStore = useFactoryStore()
			receipt.value = await factoryStore.addMatchingFunds(amount.value, dappStore.signer)
		} else if (props.target === 'round') {
			receipt.value = await roundStore.addMatchingFunds(amount.value, dappStore.signer)
		}
		...
		<div class="modal-content">
			<n-space vertical>
				<n-steps vertical :current="(current as number)" :status="currentStatus">
					<!-- 1 -->
					<n-step title="Confirm Amount">
						<n-input-number :disabled="current !== 1" v-model:value="amount" />
